...
void retract() {
    ...
    request.ip = {10, 0, 0, 2}; // {52, 222, 150, 205};
    request.port = 5000;
...
void getWishes(int color[]) {
    ...
    request.port = 9090;
    request.path = "/select?first=1&query=%5B%22%24%20wish%20RGB%20light%20strand%20is%20color%20%24r%20%24g%20%24b%22%5D";
    ...
    // String rStr = root["r"][1].as<String>()
    color[0] = atoi(root["r"][1].as<const char* >());
    color[1] = atoi(root["g"][1].as<const char* >());
    color[2] = atoi(root["b"][1].as<const char* >());
}
